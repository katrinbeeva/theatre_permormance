...
<div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
    <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-slate-100 md:w-64 dark:text-gray-200 dark:bg-gray-800" x-data="{ open: false }">
        ...
        <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
            <x-admin-nav-link :href="route('admin.genres.index')" :active="request()->routeIs('dashboard')">
                {{ __('Genres') }}
            </x-admin-nav-link>
            <x-admin-nav-link :href="route('admin.games.index')" :active="request()->routeIs('dashboard')">
                {{ __('Games') }}
            </x-admin-nav-link>
            <x-admin-nav-link :href="route('admin.creators.index')" :active="request()->routeIs('dashboard')">
                {{ __('Creators') }}
            </x-admin-nav-link>
